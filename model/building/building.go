...
func (b *Building) ToBuildingUnits(x uint8, y uint8, construction bool) []BuildingUnit {
	...
	for i := uint8(0); i < numFloors; i++ {
		...
		if y == 0 || !b.Plan.HasUnit(x, y-1, i) {
			n = &BuildingWall{M: p.Floors[i].M, Windows: true, Door: false, B: b, Construction: construction}
		}
		var e *BuildingWall
		if x == BuildingBaseMaxSize-1 || !b.Plan.HasUnit(x+1, y, i) {
			e = &BuildingWall{M: p.Floors[i].M, Windows: true, Door: false, B: b, Construction: construction}
		}
		var s *BuildingWall
		if y == BuildingBaseMaxSize-1 || !b.Plan.HasUnit(x, y+1, i) {
			s = &BuildingWall{M: p.Floors[i].M, Windows: true, Door: false, B: b, Construction: construction}
		}
		var w *BuildingWall
		if x == 0 || !b.Plan.HasUnit(x-1, y, i) {
			w = &BuildingWall{M: p.Floors[i].M, Windows: true, Door: false, B: b, Construction: construction}
		}
