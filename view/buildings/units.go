...
func WallMaterialName(t building.BuildingType, m *materials.Material, shape uint8, broken bool) string {
	...
	if t == building.BuildingTypeMine && m == materials.GetMaterial("wood") {
		return "wood_old"
	}
	return m.Name
}
			...
			*/
			...
			if !unit.Construction && wall.Windows != building.WindowTypeNone {
				...
				if wall.Windows == building.WindowTypeFactory {
					...
					accents := unit.B.Plan.BuildingType == building.BuildingTypeFactory || unit.B.Plan.BuildingType == building.BuildingTypeWorkshop
					RenderWindows(cv, rf, rfIdx1, rfIdx2, z, wall.Door, wall.Windows == building.WindowTypeFrench, accents, flowers)
